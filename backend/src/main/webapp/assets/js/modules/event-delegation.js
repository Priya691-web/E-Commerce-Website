...
(function(window) {
    ...
    const EventDelegation = {
        ...
        on: function(eventType, selector, handler, options = {}) {
            ...
            // If already initialized, add the listener immediately
            if (this.initialized) {
                this.addSingleListener(eventType, selector, handler, options);
            }
        ...
        /**
         * Add a single listener to document (used for dynamic registration)
         */
        addSingleListener: function(eventType, selector, handler, options) {
            const listener = {
                eventType,
                selector,
                handler,
            ...
            // Check if document listener already exists for this event type
            if (!this.documentListeners[eventType]) {
                // Create new document listener
                this.documentListeners[eventType] = [];
                
                document.addEventListener(eventType, (event) => {
                    this.documentListeners[eventType].forEach(l => {
                        const target = event.target.closest(l.selector);
                        if (target) {
                            l.handler.call(target, event, target);
                        }
                    });
            ...
            // Add to the list
            this.documentListeners[eventType].push(listener);

            if (this.debug) {
                console.log(`EventDelegation: Added dynamic listener for ${eventType}:${selector}`);
            }
        },
