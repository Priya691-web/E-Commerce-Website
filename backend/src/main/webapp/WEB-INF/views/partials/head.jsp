...
<script nonce='<%= request.getAttribute("cspNonce") %>'>
    ...
    // Disabled for auth pages to prevent blank screen issue
    var isAuthPage = <%= (_pageCSS != null && _pageCSS.contains("auth")) ? "true" : "false" %>;
    if (!isAuthPage) {
...
<%-- Cart interactions module - handles Add to Cart buttons globally --%>
<% if (_pageCSS == null || !_pageCSS.contains("auth")) { %>
<script src="<%= request.getContextPath() %>/assets/js/modules/cart-interactions.js" defer></script>
<% } %>

<%-- WishlistManager.js - Consolidated wishlist operations - skip on auth pages to reduce loading --%>
<% if (_pageCSS == null || !_pageCSS.contains("auth")) { %>
<script src="<%= request.getContextPath() %>/assets/js/managers/WishlistManager.js" defer></script>
<% } %>
