...
<aside class="account-sidebar">
    <nav class="account-nav" aria-label="Account navigation">
        <a href="<%= ctx %>/account/profile" class="account-nav-item <%= currentPath.endsWith("/account/profile") || currentPath.equals(ctx + "/account/profile") ? "active" : "" %>">
            <span class="nav-icon" aria-hidden="true">
                <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M20 21v-2a4 4 0 0 0-4-4H8a4 4 0 0 0-4 4v2"/><circle cx="12" cy="7" r="4"/></svg>
            </span>
            <span>Profile</span>
        </a>
        <a href="<%= ctx %>/account/profile/edit" class="account-nav-item <%= currentPath.endsWith("/edit") ? "active" : "" %>">
            <span class="nav-icon" aria-hidden="true">
                <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M11 4H4a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2v-7"/><path d="M18.5 2.5a2.121 2.121 0 0 1 3 3L12 15l-4 1 1-4 9.5-9.5z"/></svg>
            </span>
            <span>Edit Profile</span>
        </a>
        <a href="<%= ctx %>/account/addresses" class="account-nav-item <%= currentPath.contains("/account/address") ? "active" : "" %>">
            <span class="nav-icon" aria-hidden="true">
                <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M21 10c0 7-9 13-9 13s-9-6-9-13a9 9 0 0 1 18 0z"/><circle cx="12" cy="10" r="3"/></svg>
            </span>
            <span>Addresses</span>
            <% if (addrCount > 0) { %><span class="nav-badge"><%= addrCount %></span><% } %>
        </a>
        <a href="<%= ctx %>/account/profile/settings" class="account-nav-item <%= currentPath.endsWith("/settings") ? "active" : "" %>">
            <span class="nav-icon" aria-hidden="true">
        ...
        <a href="<%= ctx %>/orders" class="account-nav-item <%= currentPath.endsWith("/orders") || currentPath.contains("/orders") ? "active" : "" %>">
            <span class="nav-icon" aria-hidden="true">
            ...
            <span>Orders</span>
            <% if (orderCount > 0) { %><span class="nav-badge"><%= orderCount %></span><% } %>
        </a>
        <a href="<%= ctx %>/wishlist" class="account-nav-item <%= currentPath.endsWith("/wishlist") || currentPath.contains("/wishlist") ? "active" : "" %>">
            <span class="nav-icon" aria-hidden="true">
